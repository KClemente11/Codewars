...
function distanceCycled(cadence, gear, time) {
    let totalCadence = cadence * time
    let gearRatio = gear.split("x")
    let development = 2.11115 * (gearRatio[0] / gearRatio[1])
    
    let distance =  development * totalCadence / 1000
    
    return `You cycled ${distance.toFixed(3)} kilometers!`
  }