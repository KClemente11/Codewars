...
function isLucky(ticket) {
  ...
  let firstHalf = ticket.slice(0,3).split("").reduce((prev, curr) => {return Number(prev) + Number(curr) })
  let secondHalf = ticket.slice(3).split("").reduce((prev, curr) =>  {return Number(prev) + Number(curr) })
